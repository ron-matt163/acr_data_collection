...
def get_golang_functions_in_range(code, start_line, end_line):
    parser = tree_sitter.Parser()
    golang_language = LANGUAGE_MAP.get('golang')
    if golang_language is None:
        print("golang language could not be loaded.")
        return None
    else:
        print("golang language loaded successfully.")

    parser.set_language(golang_language)
...
file_contents = ""
with open("sample_go_code.go", 'r') as file:
    file_contents = file.read()

functions = get_golang_functions_in_range(file_contents, 91, 158)
